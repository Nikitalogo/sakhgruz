...
const SLIDES = [
  { label: 'ОТЗЫВЫ',  num: '01' },
  { label: 'ВАКАНСИИ', num: '02' },
  { label: 'КОНТАКТЫ', num: '03' },
]
...
export default function HorizontalSlider({ services, reviews, settings }: Props) {
  ...
  // Listen for nav clicks from Navbar (indices 0=ОТЗЫВЫ, 1=ВАКАНСИИ, 2=КОНТАКТЫ)
  useEffect(() => {
  ...
  return (
    <>
      {/* ── Статичная сетка услуг ── */}
      <section id="services" style={{
        background: '#FFF4E6',
        padding: isMobile
          ? '48px 16px 40px'
          : isTablet
          ? '72px clamp(20px,4vw,36px) 56px'
          : '96px clamp(20px,4vw,44px) 72px',
      }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          ...
          <div style={{
            ...
            {services.map((s, i) => (
              <ServiceCard key={s.id} s={s} i={i} isMobile={isMobile} />
            ))}
          </div>
        </div>
      </section>

      {/* ── Слайдер: Отзывы / Вакансии / Контакты ── */}
...
function ServiceCard({ s, i, isMobile }: { s: Service; i: number; isMobile: boolean }) {
