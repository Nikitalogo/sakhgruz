  ...
  return (
    <div id="slider" style={S}>
      ...
      <div style={{
        ...
        backgroundImage: `repeating-linear-gradient(90deg,transparent,transparent 79px,rgba(255,255,255,.01) 79px,rgba(255,255,255,.01) 80px),repeating-linear-gradient(0deg,transparent,transparent 79px,rgba(255,255,255,.01) 79px,rgba(255,255,255,.01) 80px)`,
      }}/>

      {/* Progress */}
      <div style={{
        position: 'absolute', top: 0, left: 0, height: 2, background: '#F97316', zIndex: 5,
          ...
          {[{ dir: -1, path: 'M15 18l-6-6 6-6' }, { dir: 1, path: 'M9 18l6-6-6-6' }].map(({ dir, path }) => (
            <button key={dir} onClick={() => goTo(current + dir)}
              ...
              style={{
                ...
                borderRight: dir === -1 ? '1px solid #1e1e1e' : 'none',
                background: 'transparent', cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                opacity: (dir === -1 ? current === 0 : current === SLIDES.length - 1) ? .2 : 1,
                transition: 'all .2s',
              }}
              onMouseEnter={e => { if (!(e.currentTarget as HTMLButtonElement).disabled) (e.currentTarget as HTMLButtonElement).style.background = '#F97316' }}
              onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'transparent' }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#e8e0d0" strokeWidth="2.5">
                <path d={path}/>
              </svg>
            </button>
          ))}
      ...
      <div style={panel}
        ...
        {current === 1 && (
          <>
            <SectionHead num="02" title="ОТЗЫВЫ"/>
            <div style={{
              display: 'flex', alignItems: 'center', gap: 24, marginBottom: 36,
              border: '1px solid #1e1e1e', padding: '20px 28px',
            }}>
              <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 64, color: '#F97316', lineHeight: 1 }}>5.0</span>
              <div>
                <div style={{ color: '#F97316', fontSize: 18, letterSpacing: 3 }}>★★★★★</div>
              ...
              <a href="https://2gis.ru/yuzhnosakhalinsk/firm/70000001099199325/tab/reviews"
                ...
                style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: '#F97316', textDecoration: 'none', letterSpacing: 2, marginLeft: 'auto', opacity: .8 }}>
                Все отзывы →
              </a>
            </div>
            <div className="reviews-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', flex: 1 }}>
              {reviews.map((r, i) => (
                <div key={r.id ?? i} style={{ border: '1px solid #1e1e1e', padding: 36, margin: -1 }}>
                  ...
                  <p style={{ fontFamily: "'Oswald',sans-serif", fontSize: 15, fontWeight: 300, color: '#e8e0d0', lineHeight: 1.75, marginBottom: 20 }}>{r.text}</p>
                  <div style={{ color: '#F97316', fontSize: 11, letterSpacing: 2, marginBottom: 8 }}>{'★'.repeat(r.rating ?? 5)}</div>
                  <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: '#F97316', letterSpacing: 1 }}>{r.author}</div>
                  {r.review_date && <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: '#444', letterSpacing: 1, marginTop: 4 }}>{r.review_date}</div>}
                </div>
              ))}
            </div>
          </>
            ...
            <div className="vacancies-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1 }}>
              {VACANCIES.map(v => (
                <div key={v.id} style={{ border: '1px solid #1e1e1e', padding: 44, margin: -1, position: 'relative', overflow: 'hidden' }}
                  className="vac-card">
                  <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: '#F97316', letterSpacing: 2, marginBottom: 12, display: 'block' }}>{v.id}</span>
                  <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 52, letterSpacing: 2, color: '#e8e0d0', lineHeight: 1, marginBottom: 8 }}>{v.title}</div>
                  <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 13, color: '#F97316', letterSpacing: 1, marginBottom: 24 }}>{v.salary}</div>
                  <p style={{ fontFamily: "'Oswald',sans-serif", fontSize: 14, fontWeight: 300, color: '#666', lineHeight: 1.6, marginBottom: 24 }}>{v.desc}</p>
                  <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 32 }}>
                    {v.conditions.map((c, i) => (
                      <li key={i} style={{ fontFamily: "'Oswald',sans-serif", fontSize: 13, fontWeight: 300, color: '#e8e0d0', display: 'flex', gap: 10 }}>
                  ...
                  onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = '#F97316'; (e.currentTarget as HTMLAnchorElement).style.color = '#000' }}
                  onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'transparent'; (e.currentTarget as HTMLAnchorElement).style.color = '#F97316' }}>
                    Позвонить по вакансии
                  </a>
                </div>
              ))}
              <div style={{
                gridColumn: '1/-1', border: '1px solid #1e1e1e', padding: '24px 44px',
                margin: -1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20,
              }}>
                <span style={{ fontFamily: "'Oswald',sans-serif", fontSize: 14, fontWeight: 300, color: '#666' }}>
                  Работа в Южно-Сахалинске · <strong style={{ color: '#e8e0d0', fontWeight: 400 }}>Неполная или полная занятость</strong> · Оплата без задержек
                </span>
                <a href="https://2gis.ru/yuzhnosakhalinsk/firm/70000001099199325/tab/vacancies"
                  target="_blank" rel="noopener noreferrer"
                  style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: '#F97316', textDecoration: 'none', letterSpacing: 2, whiteSpace: 'nowrap', opacity: .8 }}>
                  Вакансии на 2ГИС →
                </a>
              </div>
            </div>
            ...
            <div style={{ border: '1px solid #1e1e1e', flex: 1 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
                {[
                  ...
                  <div key={label} style={{ padding: 'clamp(32px,4vw,52px) clamp(20px,4vw,44px)', borderRight: '1px solid #1e1e1e', borderBottom: '1px solid #1e1e1e' }}>
                    <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: '#F97316', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 12, display: 'block' }}>{label}</span>
                    {href
                      ? <a href={href} style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(28px,4vw,56px)', letterSpacing: 2, color: '#e8e0d0', textDecoration: 'none', display: 'block', lineHeight: 1.1, transition: 'color .2s' }}
                          onMouseEnter={e => (e.currentTarget as HTMLAnchorElement).style.color = '#F97316'}
                          onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.color = '#e8e0d0'}>{val}</a>
                      : <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(28px,4vw,56px)', letterSpacing: 2, color: '#e8e0d0', display: 'block', lineHeight: 1.1 }}>{val}</span>
                    }
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
                {[
                  ...
                  <a key={name} href={href} style={{
                    padding: 'clamp(16px,2vw,22px) clamp(20px,4vw,44px)',
                    borderRight: '1px solid #1e1e1e', display: 'flex', alignItems: 'center', gap: 14,
                  ...
                  onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.background = 'transparent'}>
                    <div style={{ width: 36, height: 36, border: '1px solid #1e1e1e', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all .22s', color: '#666' }}>
                      {icon}
                    </div>
                    <div>
                      <div style={{ fontFamily: "'Oswald',sans-serif", fontSize: 15, color: '#e8e0d0' }}>{name}</div>
                      {href === '#' && <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 8, color: '#444', letterSpacing: 1.5, marginTop: 2 }}>СКОРО · ДОБАВИТЬ В НАСТРОЙКАХ</div>}
                    </div>
                  </a>
                ))}
              </div>
        ...
        <div style={{
          ...
          <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, letterSpacing: 5, color: '#444' }}>
            САХ<span style={{ color: '#F97316' }}>ГРУЗ</span>
          </span>
          <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: '#2e2e2e', letterSpacing: 2, textTransform: 'uppercase' as const }}>
            © 2026 · Южно-Сахалинск
          </span>
        </div>
      </div>
...
function ServiceCard({ s, i, active, onHover }: { s: Service; i: number; active: boolean; onHover: () => void }) {
  ...
  return (
    <div
      ...
      style={{
        border: '1px solid #1e1e1e', padding: '36px 30px', margin: -1,
        cursor: 'pointer', position: 'relative', overflow: 'hidden',
        background: on ? '#F97316' : 'transparent', transition: 'background .36s cubic-bezier(.76,0,.24,1)',
      }}>
      <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: on ? 'rgba(0,0,0,.35)' : '#F97316', letterSpacing: 2, marginBottom: 10, display: 'block', transition: 'color .3s' }}>
